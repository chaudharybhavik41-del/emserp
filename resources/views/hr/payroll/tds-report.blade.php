...
<div class="container-fluid">
    <div class="d-flex flex-wrap justify-content-between align-items-start gap-2 mb-3">
        <div>
            <h3 class="mb-1"><i class="bi bi-percent"></i> TDS Report</h3>
    ...
    <div class="row g-3 mb-3">
        ...
        <div class="col-md-8">
            <div class="card border-0 shadow-sm h-100">
                <div class="card-body">
                    <div class="text-muted small">Total TDS</div>
                    <div class="fs-5 fw-bold">₹{{ number_format($summary['total_tds'] ?? 0, 2) }}</div>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="card border-0 shadow-sm">
        <div class="card-header bg-white">
            <i class="bi bi-table"></i> TDS Deductions
        </div>
                ...
                <tbody>
                    @forelse($payrolls as $p)
                        <tr>
                            ...
                            <td class="text-end fw-semibold">{{ number_format($p->tds ?? 0, 2) }}</td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="4" class="text-center text-muted py-4">No TDS rows found.</td>
                        </tr>
                    @endforelse
                </tbody>
                ...
                @if(!empty($payrolls) && count($payrolls) > 0)
                    <tfoot class="table-light">
                        <tr>
                            ...
                            <th class="text-end">{{ number_format($summary['total_tds'] ?? 0, 2) }}</th>
                        </tr>
                    </tfoot>
                @endif
            </table>
