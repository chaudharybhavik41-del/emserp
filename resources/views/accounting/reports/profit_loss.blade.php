    ...
    $ledgerTo = optional($toDate)->toDateString();
                ...
                <div class="card-body">
                    <form method="GET" class="row g-3 align-items-end">
                        <div class="col-md-3">
                            ...
                            <input type="date" name="from_date" value="{{ request('from_date', $ledgerFrom) }}"
                                class="form-control form-control-sm">
                        </div>

                        <div class="col-md-3">
                            <label class="form-label form-label-sm">To date</label>
                            <input type="date" name="to_date" value="{{ request('to_date', $ledgerTo) }}"
                                class="form-control form-control-sm">
                        </div>
                        ...
                        <div class="col-md-7">
                            ...
                            <div class="input-group input-group-sm">
                                ...
                                <input type="text" id="plLedgerSearch" class="form-control"
                                    placeholder="Filter income and expense ledgers...">
        ...
        $collapseId = 'pl_income_' . ($g['group']->id ?? 'x' . $loop->index);
                                ...
                                <div class="border rounded mb-3 pl-group" data-group-key="{{ $groupKey }}">
                                    <div
                                        class="bg-light px-3 py-2 small fw-semibold d-flex align-items-center justify-content-between">
                                        ...
                                        <div class="text-end">
                                            {{-- Ledger count --}}
                                            <span class="text-muted me-3">{{ count($g['accounts']) }} ledgers</span>

                                            {{-- Total amount --}}
                                            <span class="fw-bold text-primary">
                                                Total {{ $g['group']->name }} : {{ number_format($g['total'], 2) }}
                                            </span>
                                        </div>
                                    </div>

                                    <div id="{{ $collapseId }}" class="collapse show pl-group-collapse"
                                        data-group-key="{{ $groupKey }}">
                                        <div class="p-2">
                                            <table class="table table-sm mb-0">
                                                <tbody>
                                                    @foreach($g['accounts'] as $row)
                                                                                        <tr class="pl-ledger-row" data-group-key="{{ $groupKey }}"
                ...
                'to_date' => $ledgerTo,
                                                    ...
                                                    @endforeach
                                                    <tr class="table-light fw-semibold pl-group-total"
                                                        data-group-key="{{ $groupKey }}">
