...
use App\Services\Tally\TallyPurchaseBillXmlService;
...
class PurchaseBillController extends Controller
{
    ...
    protected function purchaseBillIndexQuery(Request $request): Builder
    {
        $query = PurchaseBill::with([
            ...
            'lines.materialReceipt.purchaseOrder'
        ])
            ->orderByDesc('posting_date')
            ->orderByDesc('bill_date')
            ->orderByDesc('id');
            ...
            $query->where(function ($q) use ($search) {
                $q->where('bill_number', 'like', "%{$search}%")
                    ...
                    ->orWhereHas('purchaseOrder', function ($qpo) use ($search) {
                        $qpo->where('code', 'like', "%{$search}%");
                    })
                    ->orWhereHas('lines.materialReceipt.purchaseOrder', function ($qpo2) use ($search) {
                        $qpo2->where('code', 'like', "%{$search}%");
                    });
            });
        }
    ...
    public function index(Request $request)
    {
        $query = $this->purchaseBillIndexQuery($request);
    ...
    public function exportTallyXml(Request $request, TallyPurchaseBillXmlService $tallyService)
    {
        $bills = $this->purchaseBillIndexQuery($request)
            ->where('status', 'posted')
            ->get();

        if ($bills->isEmpty()) {
            return back()->with('error', 'No posted purchase bills matched the selected filters for Tally export.');
        }

        try {
            $xml = $tallyService->buildForBills($bills);
        } catch (\Throwable $e) {
            return back()->with('error', 'Tally export failed: ' . $e->getMessage());
        }
        ...
        return response()->streamDownload(function () use ($xml) {
            echo $xml;
        }, "purchase-bills-tally-{$timestamp}.xml", [
            'Content-Type' => 'application/xml; charset=UTF-8',
        ]);
    }

    public function downloadTallyXml(PurchaseBill $bill, TallyPurchaseBillXmlService $tallyService)
    {
        try {
            $xml = $tallyService->buildForBill($bill);
        } catch (\Throwable $e) {
            return redirect()
                ->route('purchase.bills.show', $bill)
                ->with('error', 'Tally export failed: ' . $e->getMessage());
        }

        $safeBillNumber = preg_replace('/[^A-Za-z0-9._-]+/', '-', (string) ($bill->bill_number ?: ('bill-' . $bill->id)));
    ...
    /**
     * Get GRNs associated with a purchase bill.
     */
    public function getGrns(PurchaseBill $bill)
    {
        $bill->load([
            'lines.materialReceipt.supplier',
            'lines.materialReceipt.purchaseOrder',
            'lines.item.uom'
        ]);

        $lines = $bill->lines->whereNotNull('material_receipt_id')->map(function ($line) {
            return [
                'grn_id'         => $line->materialReceipt->id,
                'receipt_number' => $line->materialReceipt->receipt_number,
                'receipt_date'   => $line->materialReceipt->receipt_date,
                'supplier_name'  => $line->materialReceipt->supplier?->legal_name ?: $line->materialReceipt->supplier?->name,
                'po_code'        => $line->materialReceipt->purchaseOrder?->code,
        ...
        if (Schema::hasColumn('material_receipts', 'purchase_order_id')) {
            $ordersQuery->whereExists(function ($query) use ($supplierId) {
                $query->selectRaw('1')
                    ...
                    // Join with purchase_bill_lines and purchase_bills to calculate billed quantity
                    ...
                    ->groupBy('mrl.id', 'mrl.qty_pcs', 'mrl.received_weight_kg')
                    // Only show PO if there is at least one GRN line that is not yet fully billed (ignoring cancelled bills)
                    ->havingRaw("(CASE WHEN COALESCE(mrl.received_weight_kg, 0) > 0 THEN mrl.received_weight_kg ELSE COALESCE(mrl.qty_pcs, 0) END) - SUM(CASE WHEN pb.status = 'cancelled' THEN 0 ELSE COALESCE(pbl.qty, 0) END) > 0.0001");
            });
        }
