...
@section('content')
    ...
    <div class="container-fluid">
        ...
        <p class="text-muted small mb-3">
            Date filter is applied on <strong>Posting Date</strong> (books/voucher date). Invoice Date is shown separately
            for GST compliance.
        </p>
            ...
            <div class="card-body">
                <form method="GET" class="row g-3 align-items-end">
                    ...
                    <div class="col-md-6 d-flex gap-2 flex-wrap">
                        ...
                        <a class="btn btn-sm btn-outline-secondary"
                            href="{{ route('accounting.reports.gst-purchase-register') }}">Reset</a>
                    </div>
                ...
                <div class="card">
                    <div class="card-body">
                            ...
                            <table class="table table-sm mb-0">
                                <tbody>
                                    <tr>
                                        ...
                                        <td class="text-end">
                                            {{ \App\Support\MoneyHelper::fromPaise($totals['taxable'] ?? 0) }}</td>

                                        <th class="text-muted" style="width: 120px;">CGST</th>
                                        <td class="text-end">{{ \App\Support\MoneyHelper::fromPaise($totals['cgst'] ?? 0) }}
                                        </td>

                                        <th class="text-muted" style="width: 120px;">SGST</th>
                                        <td class="text-end">{{ \App\Support\MoneyHelper::fromPaise($totals['sgst'] ?? 0) }}
                                        </td>

                                        <th class="text-muted" style="width: 120px;">IGST</th>
                                        <td class="text-end">{{ \App\Support\MoneyHelper::fromPaise($totals['igst'] ?? 0) }}
                                        </td>
                                    </tr>
                                    <tr>
                                        <th class="text-muted">RCM CGST</th>
                                        <td class="text-end">
                                            {{ \App\Support\MoneyHelper::fromPaise($totals['rcm_cgst'] ?? 0) }}</td>

                                        <th class="text-muted">RCM SGST</th>
                                        <td class="text-end">
                                            {{ \App\Support\MoneyHelper::fromPaise($totals['rcm_sgst'] ?? 0) }}</td>

                                        <th class="text-muted">RCM IGST</th>
                                        <td class="text-end">
                                            {{ \App\Support\MoneyHelper::fromPaise($totals['rcm_igst'] ?? 0) }}</td>
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-end">
                                            {{ \App\Support\MoneyHelper::fromPaise($totals['invoice'] ?? 0) }}</td>

                                        <th class="text-muted">TCS</th>
                                        <td class="text-end">
                                            {{ \App\Support\MoneyHelper::fromPaise($totals['tcs_amount'] ?? 0) }}</td>

                                        <th class="text-muted">TDS</th>
                                        <td class="text-end">
                                            {{ \App\Support\MoneyHelper::fromPaise($totals['tds_amount'] ?? 0) }}</td>

                                        <th class="text-muted">Net Payable</th>
                                        <td class="text-end">
                                            {{ \App\Support\MoneyHelper::fromPaise($totals['net_payable'] ?? 0) }}</td>
                                    </tr>
                                    <tr>
                            ...
                            <table class="table table-sm table-striped align-middle mb-0">
                                ...
                                <tbody>
                                    <tr id="gprNoMatch" class="d-none">
                                        <td colspan="17" class="text-center text-muted py-4">No bills match the search text.
                                        </td>
                                    </tr>
                                    @if(count($bills))
                                        @foreach($bills as $bill)
                                            ...
                                            <tr class="gpr-row" data-row-text="{{ $searchText }}">
                                                ...
                                                <td>{{ \Carbon\Carbon::parse($postingDate)->format('d-m-Y') }}</td>
                                                <td>{{ $bill->bill_number }}</td>
                                                <td>
                                                    @if($bill->voucher)
                                                        <a href="{{ route('accounting.vouchers.show', $bill->voucher) }}"
                                                ...
                                                <td >
                                                    @if($bill->supplier)
                                                        <a href="{{ route('accounting.reports.supplier-ageing', ['supplier_id' => $bill->supplier->id, 'as_of_date' => $periodTo]) }}"
                                                            class="text-decoration-none">
                                                            {{ $bill->supplier->name }}
                                                        </a>
                                                ...
                                                <td>
                                                    <span
                                                        class="badge bg-{{ $bill->status === 'posted' ? 'success' : ($bill->status === 'draft' ? 'secondary' : 'danger') }}">
                                                        {{ $bill->status }}
                                                    </span>
                                                </td>
                                            </tr>
                        ...
                        <p class="text-muted small mt-3 mb-0">
                            ...
                            <br>• If you post manual GST vouchers (without Purchase Bills), they will not appear here (we
                            can add a voucher-based register in a later phase if required).
                        </p>
                    </div>
                </div>
            </div>
